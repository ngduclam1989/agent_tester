...
2. Page Object Model — locator có đặt đúng trong Page, assertion có đúng trong Test không?
3. Code smell — logic trùng lặp, method quá dài, magic numbers?
4. Maintainability — dễ bảo trì khi UI thay đổi không?
5. Reusability — có tận dụng helper/utils tái sử dụng không?
6. Locator strategy — locator có ổn định, đúng thứ tự ưu tiên (`.claude/rules/locator_strategy.md`) không?
7. Wait strategy — có hard sleep không? Có dùng smart waits đúng cách không?
8. Test stability — test có độc lập, data có unique, assertion có đủ không?
9. Clean code — có sót debug log, commented code, unused variable không?

Trả về kết quả dạng bảng:

| # | Vấn đề | File/Dòng | Mức độ | Đề xuất sửa |

Kèm theo: tóm tắt đánh giá tổng thể (score /10), top 3 vấn đề nghiêm trọng nhất cần sửa ngay, và ví dụ code refactor cho các vấn đề mức High.

Triggers when user asks:

- review automation code
- review test automation
- đánh giá code automation
...
- Page Object classes
- Test classes structured with the Arrange → Act → Assert pattern (setup data/state → perform action → verify result)
