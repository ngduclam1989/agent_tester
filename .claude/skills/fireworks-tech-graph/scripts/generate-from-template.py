#!/usr/bin/env python3
"""
Bộ sinh biểu đồ SVG dựa trên phong cách (style).

Cách dùng:
  python3 generate-from-template.py <template-type> <output-path> [data-json]

Bộ sinh này thực hiện nhiều việc hơn là chỉ "điền thông tin vào mẫu".
Nó mã hóa ngôn ngữ hình ảnh từ các tài liệu hướng dẫn phong cách để đầu ra
đạt chất lượng trình diễn (showcase) tiệm cận hơn so với bộ kết xuất chung trước đây.
"""
...
def main() -> None:
    if len(sys.argv) < 3:
        print("Cách dùng: python3 generate-from-template.py <template-type> <output-path> [data-json]")
        sys.exit(1)
    ...
    try:
        ...
        print(f"✓ Đã tạo file SVG: {output_path}")
    except FileNotFoundError as exc:
        print(f"Lỗi: {exc}")
        sys.exit(1)
    except json.JSONDecodeError as exc:
        print(f"Lỗi: Định dạng JSON không hợp lệ: {exc}")
        sys.exit(1)
