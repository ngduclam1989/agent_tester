...
## 13. Quy ước output cho project này (Garage Automation Testing Kit)

> Đây là bổ sung riêng cho repo này — không phải nội dung gốc của skill (nguồn: [github.com/cathrynlavery/diagram-design](https://github.com/cathrynlavery/diagram-design)).

`diagram-design` là skill vẽ diagram chính thức của project — thay thế `fireworks-tech-graph` (đã gỡ bỏ) và cách vẽ nháp bằng Mermaid text trước đây trong `rbt_manual_testing`.

- **Output đi vào `practices/diagram/`** — cùng thư mục với các diagram hiện có (`wave06-*-activity.svg`), giữ **phẳng** (không tạo sub-folder theo wave trừ khi user yêu cầu khác).
- **Tên file**: kebab-case, mô tả chức năng + loại sơ đồ, ví dụ `wave07-partner-link-sequence.html`, `onboard-tenant-state.html`.
- Deliverable mặc định là `.html` tự chứa (theo §12). Chỉ export thêm `.svg`/`.png` khi user yêu cầu rõ (§ Exporting to PNG / SVG).
- Diagram vẽ nháp trong quy trình `rbt_manual_testing` / `/generate_manual_testcases_rbt` (Bước 1 mục 6) vẫn phải qua bước **user confirm** trước khi dùng làm căn cứ phân tích ở Bước 2 — không đổi so với quy trình cũ, chỉ đổi công cụ vẽ.
