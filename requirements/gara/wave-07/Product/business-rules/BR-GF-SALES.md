...
| BR-CROSS-006 (v3, 2026-08-10 — chốt Kafka theo ADR-029) | Tích hợp Driver+ dùng Kafka hai chiều trên `AC-DEV-BOOKING-EVENTS`, `MessageGroup=BOOKING`. Giữ các step production `BOOKING.CREATE.REQUEST`, `BOOKING.CANCELLED`, `BOOKING.CREATE.RESPONSE`, `BOOKING.UPDATE.RESPONSE` và `BOOKING.CHANGE.STATUS`; bổ sung `BOOKING.CANCEL.RESPONSE`. Response lỗi luôn theo event correlation, không dùng HTTP đồng bộ. Emit phiếu DV/QT vẫn chờ contract riêng tại `FEAT-SO-DETAIL` BR-SO-DTL-007 và `FEAT-STL-CREATE` BR-STL-CRE-008. | Driver+ (external) | Consumer bắt buộc filter `MessageGroup` + `MessageStep`; outbox/inbox mandatory (Critical Rule #2) hai chiều. |
...
| BR-BOOK-024 (v2, 2026-08-03 — thu hep pham vi F4) | Moi su kien outbound sang Driver+ **do `gf-sales` phat hanh** (tao moi booking, cap nhat trang thai, emit phieu dich vu — xem `FEAT-SO-DETAIL` BR-SO-DTL-007) tuan thu outbox/inbox mandatory (Critical Rule #2) — trang thai/du lieu cuc bo phia GMS KHONG rollback khi gui that bai; retry doc lap voi transaction nghiep vu da commit. **Khong bao gom** emit phieu quyet toan — thuoc `gf-accounting`, xem BR-STL-CRE-008 + CB-ACC-008 (outbox pattern rieng, doc lap). | Integration | FEAT-BOOK-DRIVERPLUS-OUTBOUND, FEAT-SO-DETAIL |
