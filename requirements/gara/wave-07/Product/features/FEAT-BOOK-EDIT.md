...
- [ ] **AC-15**: Đồng bộ thông tin cập nhật sang Driver+
  ...
  - Thì: hệ thống đồng bộ thông tin lịch hẹn đã cập nhật sang Driver+ qua Kafka event `BOOKING.UPDATE.RESPONSE` (giữ nguyên step production theo ADR-029). Khách hàng trên ứng dụng Driver+ nhận được thông tin lịch hẹn mới nhất.
  - **Không đổi** trong đợt viết lại tích hợp Driver+ 2026-08-03 — AC này chỉ cover đồng bộ khi **sửa nội dung** (không đổi trạng thái vòng đời). Đồng bộ **trạng thái vòng đời** (xác nhận/từ chối/xe đến/hủy) nay thuộc `FEAT-BOOK-DRIVERPLUS-OUTBOUND.md`.
...
- **BR-BOOK-EDIT-001** (≡ `BR-BOOK-007` tại `BR-GF-SALES.md` §2.1 — 2 ID cùng 1 nội dung, sửa 1 chỗ phải sửa cả 2, BA-review Wave 7 F2): Chỉ cho phép chỉnh sửa lịch hẹn khi trạng thái là **"Lịch hẹn mới"** hoặc **"Đã xác nhận"**. Các trạng thái khác không hiển thị nút chỉnh sửa.
...
| 2026-08-03 | 3 | user (Business Authority) qua main agent | Thêm ghi chú làm rõ AC-15 **không đổi** trong đợt viết lại tích hợp Driver+ — chỉ cover đồng bộ khi sửa nội dung, phân biệt với đồng bộ trạng thái vòng đời nay thuộc `FEAT-BOOK-DRIVERPLUS-OUTBOUND.md`. |
| 2026-08-03 | 4 | user (Business Authority) qua main agent | **Fix P2 (BA-review round 1)**: Metadata table `Status` sửa "PLANNED" → **DONE** khớp frontmatter (drift template có sẵn từ trước, thuần editorial). |
| 2026-08-03 | 5 | user (Business Authority) qua main agent | **Fix F2 (BA-review Wave 7)**: BR-BOOK-EDIT-001 thêm cross-ref (≡) sang `BR-BOOK-007` tại `BR-GF-SALES.md` §2.1 (ID trùng nội dung, legacy baseline) — chỉ nối cross-ref 2 chiều, không xoá/gộp ID. |
| 2026-08-10 | 6 | Business Authority qua main agent | **Chốt Kafka theo ADR-029**: AC-15 xác nhận giữ `BOOKING.UPDATE.RESPONSE` cho đồng bộ khi sửa nội dung booking; gỡ marker Architecture về tên event. |
