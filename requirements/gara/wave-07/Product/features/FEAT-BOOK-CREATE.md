...
> **AC-23 + AC-24 SUPERSEDED (2026-08-03)** — tích hợp Driver+ đã được viết lại hoàn toàn theo tài liệu mới từ đội Driver+ (FEAT-DP-034/035), tách thành 2 feature riêng vì đây là luồng tự động (event-driven) khác hẳn luồng tạo thủ công qua form của feature này:
> - Nhận yêu cầu đặt lịch từ Driver+ → xem `FEAT-BOOK-DRIVERPLUS-INBOUND.md` AC-1..5 (nay đủ 14 trường chi tiết, không chỉ mô tả chung chung).
> - Nhận yêu cầu hủy từ Driver+ → xem `FEAT-BOOK-DRIVERPLUS-INBOUND.md` AC-6..8 (nay có gate trạng thái tường minh + ghi `cancel_source`).
>
> Số AC-23/AC-24 giữ nguyên trong lịch sử (không tái sử dụng cho nội dung khác) để tránh nhầm lẫn khi tra cứu Change Log cũ.
...
- **BR-BOOK-005** (rewrite 2026-08-03 — xem chi tiết đầy đủ tại `FEAT-BOOK-DRIVERPLUS-INBOUND.md` §5): Lịch hẹn từ Driver+ được tạo qua kênh sự kiện tự động, đủ 14 trường — garage không nhập liệu mà chỉ xác nhận hoặc từ chối (xem `FEAT-BOOK-CONFIRM`, `FEAT-BOOK-DECLINE`). Phản hồi/đồng bộ sang Driver+ nay thuộc `FEAT-BOOK-DRIVERPLUS-OUTBOUND.md`.
...
| 2026-08-03 | 8 | user (Business Authority) qua main agent | **SUPERSEDE AC-23 + AC-24** (Nhóm A2, luồng Driver+) — tích hợp Driver+ viết lại hoàn toàn theo tài liệu mới từ đội Driver+, tách thành 2 feature riêng `FEAT-BOOK-DRIVERPLUS-INBOUND` (nhận đặt lịch/hủy) + `FEAT-BOOK-DRIVERPLUS-OUTBOUND` (phản hồi/đồng bộ). Giữ số AC-23/24 trong lịch sử, không tái sử dụng. Cập nhật BR-BOOK-005 trỏ sang FEAT mới. AC-25 (Garage Care) + AC-26 (walk-in) không đổi. |
| 2026-08-03 | 9 | user (Business Authority) qua main agent | **Fix P2 (BA-review round 1)**: Metadata table `Status` sửa "PLANNED" → **DONE** khớp frontmatter (drift template có sẵn từ trước, thuần editorial). |
