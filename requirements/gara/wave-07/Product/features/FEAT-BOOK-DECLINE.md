...
- **BR-BOOK-DEC-001** (≡ phần đầu `BR-BOOK-015` tại `BR-GF-SALES.md` §2.1 — BA-review Wave 7 F2): Chỉ cho phép từ chối lịch hẹn ở trạng thái **"Lịch hẹn mới"**.
- **BR-BOOK-DEC-002** (≡ phần sau `BR-BOOK-015` tại `BR-GF-SALES.md` §2.1 — BA-review Wave 7 F2): Sau khi từ chối, thông tin từ chối (lý do) được gửi cho khách hàng qua ứng dụng Driver+ (nếu lịch hẹn có nguồn từ Driver+) — cơ chế gửi cụ thể nay thuộc `FEAT-BOOK-DRIVERPLUS-OUTBOUND.md` AC-2 (2026-08-03).
- **BR-BOOK-DEC-003** (≡ `BR-BOOK-016` tại `BR-GF-SALES.md` §2.1 — BA-review Wave 7 F2): Lý do từ chối được ghi nhận trong lịch sử trạng thái cùng người thực hiện và thời gian.
...
| 2026-08-03 | 2 | user (Business Authority) qua main agent | BR-BOOK-DEC-002 thêm cross-ref cơ chế gửi cụ thể nay thuộc `FEAT-BOOK-DRIVERPLUS-OUTBOUND.md` (đợt viết lại tích hợp Driver+). |
...
| 2026-08-03 | 4 | user (Business Authority) qua main agent | **Fix F2 (BA-review Wave 7)**: BR-BOOK-DEC-001/002/003 thêm cross-ref (≡) sang `BR-BOOK-015`/`BR-BOOK-016` tại `BR-GF-SALES.md` §2.1 (ID trùng nội dung, legacy baseline) — chỉ nối cross-ref 2 chiều, không xoá/gộp ID. |
