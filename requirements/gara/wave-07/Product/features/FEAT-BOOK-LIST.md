...
- [ ] **AC-14**: Lịch hẹn quá hạn tự động chuyển trạng thái
  ...
  - Thì: hệ thống tự động chuyển trạng thái sang **"Đã hủy"** và ghi nhận lịch sử. Lịch hẹn hiển thị trạng thái mới trong danh sách. Hệ thống ghi nhận `cancel_source = NO_SHOW_AUTO` (mọi lịch hẹn, không phân biệt nguồn — per BR-BOOK-023); nếu lịch hẹn có nguồn từ Driver+, hệ thống thêm bước đồng bộ sang Driver+ — chi tiết xem `FEAT-BOOK-DRIVERPLUS-OUTBOUND.md` AC-4 (mới, 2026-08-03).
...
- **BR-BOOK-LIST-001** (≡ `BR-BOOK-021` tại `BR-GF-SALES.md` §2.1 — BA-review Wave 7 F2): Danh sách lịch hẹn hiển thị theo phạm vi garage hiện tại (tenant). Không hiển thị lịch hẹn của garage khác.
- **BR-BOOK-LIST-002** (≡ `BR-BOOK-018` tại `BR-GF-SALES.md` §2.1 — BA-review Wave 7 F2): Trạng thái không đến và đã hủy đều hiển thị chung là **"Đã hủy"** trên giao diện (theo KG frontend).
- **BR-BOOK-LIST-003** (≡ `BR-BOOK-017` tại `BR-GF-SALES.md` §2.1 — BA-review Wave 7 F2): Lịch hẹn quá hạn ở trạng thái **"Lịch hẹn mới"** hoặc **"Đã xác nhận"** được hệ thống tự động chuyển sang **"Đã hủy"**, ghi lịch sử và gửi thông báo.
...
| 2026-08-03 | 4 | user (Business Authority) qua main agent | AC-14 thêm cross-ref `cancel_source = NO_SHOW_AUTO` + đồng bộ sang Driver+ (đợt viết lại tích hợp Driver+) — xem `FEAT-BOOK-DRIVERPLUS-OUTBOUND.md`. |
| 2026-08-03 | 5 | user (Business Authority) qua main agent | **Fix P2 (BA-review round 1)**: Metadata table `Status` sửa "PLANNED" → **DONE** khớp frontmatter (drift template có sẵn từ trước, thuần editorial). |
| 2026-08-03 | 6 | user (Business Authority) qua main agent | **Fix F2 (BA-review Wave 7)**: BR-BOOK-LIST-001/002/003 thêm cross-ref (≡) sang ID trùng nội dung tại `BR-GF-SALES.md` §2.1 (`BR-BOOK-021`/`BR-BOOK-018`/`BR-BOOK-017`) — legacy trùng ID từ baseline, chỉ nối cross-ref 2 chiều, không xoá/gộp ID. |
