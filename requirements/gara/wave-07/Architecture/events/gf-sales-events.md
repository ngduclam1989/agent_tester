...
| Total events | 7 active outbound trên 6 distinct topics + **2 DESIGN outbound** (`ServiceOrderDocumentSync` / `ServiceOrderDocumentRevoked` trên topic mới `AC-DEV-DOCUMENT-EVENTS` — ADR-031, xem §2ter) (W05 ADR-026 v2: `ServiceOrderStatusChanged` fan-out V1+V2 topic per-tenant FF exclusive publish, single Kafka send per event) + 2 external-source inbound (Driver+); 10 inactive DTO documented §3.7 |
...
| 7c | `ServiceOrderDocumentRevoked` | **`AC-DEV-DOCUMENT-EVENTS`** (MỚI) | SO đã emit SYNC sau đó chuyển "Đã huỷ" (AC-24) | Driver+ | ≤ 30s | **DESIGN** (ad-hoc 2026-08-10, ADR-031) | step `DOCUMENT.SERVICE_ORDER.REVOKED` — xem §3.11 |
...
|---|---|---|---|---|---|---|---|
| 14 | `BookingCreateRequest` | `AC-DEV-BOOKING-EVENTS` | External: Driver+ | `BookingDriverPlusConsumer` route `MessageStep=BOOKING.CREATE.REQUEST`; inbox dedup `BOOKING_CREATED_FROM_DRIVER_PLUS`; gọi `BookingV3Service.handleBookingCreatedFromDriverPlus`; sau thành công publish `BookingCreateResponseEvent` | ≤ 5s | `consumer-only-confirmed` | shared topic với outbound `BookingStatusChanged`/`BookingCreateResponseEvent`. **W07**: payload tài liệu hoá đủ 14 trường + adapter gate reject — xem §3.8 |
...
>
> **Thuần additive**: topic mới, `MessageGroup` mới, 2 step mới. KHÔNG đụng `AC-DEV-BOOKING-EVENTS` và 4 step booking đang chạy production.
...
| E3 | Step `DOCUMENT.SERVICE_ORDER.REVOKED` | **MỚI** — emit khi SO đã SYNC sau đó "Đã huỷ" | Additive | AC-24 · ADR-031 D3 |
...
2. Ghi inbox bằng `InboxEventType.BOOKING_CREATED_FROM_DRIVER_PLUS`; duplicate bị bỏ qua qua unique constraint/inbox repository (**INBOUND AC-9** — retry mạng phía D+ chỉ tạo 1 booking).
3. Parse `data` sang `BookingCreateRequestEventData`.
4. **Adapter validation gate (W07)** — kiểm 5 trường bắt buộc + bước 15 phút của `appointmentTime`. **Fail** → KHÔNG tạo booking, publish `BOOKING.CREATE.RESPONSE` `success=false` + `ERR-BOOK-001` (§3.3), kết thúc.
5. Gọi `BookingV3Service.handleBookingCreatedFromDriverPlus` → tạo booking `status=BOOKING` ("Lịch hẹn mới"), nguồn hiển thị "Từ ứng dụng tài xế" (`BR-BOOK-001`, `BR-BOOK-003`); khớp/tạo khách theo SĐT bằng cơ chế sẵn có `BR-BOOK-004` — **không** xử lý đặc biệt cho nguồn D+ (INBOUND AC-4).
6. Sau khi tạo booking, `gf-sales` publish `BookingCreateResponseEvent` `success=true` (§3.3).
...
### 3.11 `ServiceOrderDocumentRevoked` _(outbound — MỚI 2026-08-10, DESIGN, ADR-031)_

**Trigger**: SO **đã** emit `DOCUMENT.SERVICE_ORDER.SYNC` sau đó chuyển **"Đã huỷ"** (`FEAT-SO-DETAIL` AC-24, lý do huỷ bắt buộc theo AC-22/AC-23). Đường đi thực tế: huỷ phiếu quyết toán → REST reopen SO (CB-INS-003) → SO rời "Đã quyết toán" → garage huỷ phiếu (xem ADR-031 D3 ghi chú khả đạt).

**Payload** (headers `MessageGroup=DOCUMENT`, **`MessageStep=DOCUMENT.SERVICE_ORDER.REVOKED`**, `OriginTenantId={tenantId}`, `OriginMessageCode={serviceOrderCode}`):

```json
{
  "eventId": "b2c41a77-1e55-5f70-8c92-6ad3e0f21b88",
  "eventType": "ServiceOrderDocumentRevoked",
  "eventVersion": "1.0",
  "tenantId": 5001,
  "occurredAt": "2026-08-11T02:40:00Z",
  "source": "gf-sales",
  "documentCode": "PDV-20260810-00042",
  "documentType": "SERVICE_ORDER",
  "revokedReason": "Khách đổi ý, không sửa nữa",
  "correlation": { "syncEventId": "0f0d2b5c-6f1a-5a3e-9b21-3c8e7d5a1f04" }
}
```

| Field | Type | Required | Note | Cite |
|---|---|---|---|---|
| `eventId` | UUID | ✅ | `UUIDv5(NS_DP_DOCUMENT, documentCode + "\|" + documentType + "\|REVOKED")` — khoá riêng, không đụng dedupe của SYNC | ADR-031 D5 |
| `documentCode` / `documentType` | String / Enum | ✅ | Định danh phiếu bị thu hồi | AC-24 |
| `revokedReason` | String | ✅ | Lý do huỷ do garage nhập (trường "Ghi chú" bắt buộc) | AC-22 · AC-23 |
| `correlation.syncEventId` | UUID | ✅ | `eventId` của bản SYNC gốc — D+ tìm đúng chứng từ đã ghi | ADR-031 D3 |

**Idempotency**: outbox; `eventId` deterministic per phiếu → huỷ lặp không tạo bản ghi mới phía D+.

**Critical use case**: D+ **đánh dấu thu hồi**, KHÔNG xoá — giữ audit trail hồ sơ số của xe (quyết định user 2026-08-10 Q6). Nếu D+ chưa có handler cho step này khi cutover → ship SYNC trước, REVOKED sau qua CR (`open_questions`).
