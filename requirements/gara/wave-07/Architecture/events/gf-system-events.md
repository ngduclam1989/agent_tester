...
| 7 | `PartnerLinkRequestResponse` _(W07)_ | `AC-DEV-PARTNER-LINK-EVENTS` | `gf-system` xử lý xong inbound `PARTNER_LINK.REQUEST.CREATE` — ack tạo record HOẶC từ chối tại adapter gate (single-active guard) | External: **Driver Plus** | ≤ 5s | `DESIGN` (W07) | step `PARTNER_LINK.REQUEST.RESPONSE`; correlated response thay HTTP đồng bộ (ADR-029); outbox; xem §3.12 |
...
| 15 | `PartnerLinkRequestCreate` _(W07)_ | `AC-DEV-PARTNER-LINK-EVENTS` | External: **Driver Plus** | `PartnerLinkDriverPlusConsumer` route `MessageStep=PARTNER_LINK.REQUEST.CREATE`; adapter validation gate (PC-4) + single-active guard `BR-DPL-CMN-007` **trước** khi ghi domain table; inbox dedupe `PARTNER_LINK_REQUEST_CREATE_RECEIVED`; luôn publish `PARTNER_LINK.REQUEST.RESPONSE` | ≤ 5s | `DESIGN` (W07) | shared topic với 3 step outbound — **bắt buộc** filter `MessageGroup` + `MessageStep`; xem §3.11 |
...
**Envelope**: `KafkaMessageWrapper` + headers `MessageGroup=PARTNER_LINK`, `MessageStep={step}`, `OriginTenantId={tenantId}`, `OriginMessageCode={requestCode}`; `type=BASIC_MESSAGE`; `data` là JSON string. Partition key `PartnerLink-{requestCode}`.

| Variant | `MessageStep` | Điều kiện state hợp lệ | Kết quả | Cite |
|---|---|---|---|---|
| Tạo yêu cầu | `PARTNER_LINK.REQUEST.CREATE` | Garage **chưa có** record `status='LINKED'` | Tạo record `PENDING` + publish `PARTNER_LINK.REQUEST.RESPONSE` `success=true` | CB-SYS-004 · `BR-DPL-CMN-001` · AC-25 |
| — (bị chặn) | ↑ cùng step | Garage **đã có** record `LINKED` | **KHÔNG** tạo record; publish `PARTNER_LINK.REQUEST.RESPONSE` `success=false` + `ERR-DPL-010` | `BR-DPL-CMN-007` · AC-34 |
...
Event tới record **không đúng** state hợp lệ → **bỏ qua + log warning**, KHÔNG đổi state, KHÔNG xoá lịch sử (AC-33 nhánh 2, AC-35 nhánh 2).

**Payload `PARTNER_LINK.REQUEST.CREATE`**:
```json
{
...
| `partnerAccountPhone` | String | ✅ | `FEAT` AC-9 |
| `requestedAt` | ISO-8601 | ✅ | `FEAT` AC-9 |

**Payload `PARTNER_LINK.REQUEST.WITHDRAW` / `PARTNER_LINK.UNLINK`** (cùng shape):
```json
{
...
| Field | Type | Required | Cite |
|---|---|---|---|
| `requestCode` | String | ✅ | AC-33 / AC-35 (xác định record) |
...
2. Yêu cầu `OriginTenantId`; thiếu → ack + skip (tránh retry vô hạn — parity `BookingDriverPlusConsumer`). `data.tenantId` ≠ `OriginTenantId` → fail có kiểm soát, KHÔNG side-effect (`_CONVENTIONS.md` §3.5).
...
4. **Adapter validation gate (PC-4 / BR-CORE-012)** — validate payload + enforce single-active guard **TRƯỚC** khi ghi domain table.
5. Áp transition theo bảng trên trong 1 transaction; publish response/notification event qua **outbox** cùng transaction.

**Idempotency**: inbox dedupe theo `messageId`; thêm lớp 2 là unique `(tenant_id, request_code)` ở `partner_link_request` (EC-4).

### 3.12 `PartnerLinkRequestResponse` _(outbound — DESIGN W07)_

**Trigger**: sau khi consumer xử lý xong `PARTNER_LINK.REQUEST.CREATE` (cả nhánh thành công lẫn nhánh bị adapter gate chặn).

**Payload** (headers `MessageGroup=PARTNER_LINK`, `MessageStep=PARTNER_LINK.REQUEST.RESPONSE`, `OriginTenantId={tenantId}`, `OriginMessageCode={requestCode}`):
...
|---|---|---|---|---|
| `success` | Boolean | ✅ | `true` = record `PENDING` đã tạo; `false` = bị chặn | AC-34 |
| `error.code` | String | khi `success=false` | Chỉ 1 giá trị ở W07: `ERR-DPL-010` | `ERROR-CODE-REGISTRY` §5 |
...
```
 Driver Plus                         gf-system                        garage-web / garage-mobile
     ...
     ├──────────────────────────────────►│ adapter gate (PC-4)                   │
     │                                   │ ├─ đã có LINKED? ──► ② RESPONSE       │
     │◄──────────────────────────────────┤ │   success=false + ERR-DPL-010       │
     │                                   │ └─ chưa ──► INSERT status=PENDING     │
     ...
     │    hoặc PARTNER_LINK.UNLINK       │                                       │
     ├──────────────────────────────────►│ → UNLINKED, processedByLabel="Driver Plus"
     │        (KHÔNG có noti ngược)      │   UI cập nhật ngầm, không toast        │
```

1. Driver Plus publish `PARTNER_LINK.REQUEST.CREATE` (§3.11) → `gf-system` adapter gate → tạo record `PENDING` hoặc chặn theo `BR-DPL-CMN-007`.
...
5. Chiều ngược (D+ withdraw/unlink) chỉ đổi state cục bộ, **KHÔNG** phát noti ngược.
