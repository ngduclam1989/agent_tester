...
## 2ter. Driver+ integration rewrite — additive columns (DESIGN — W07, FEAT-BOOK-DRIVERPLUS-INBOUND/OUTBOUND)

> **Status**: DESIGN — 2 cột dưới đây **chưa có trong source**. **Additive only** — không rename/drop cột nào. Cite: [ADR-029](../decisions/ADR-029-driver-plus-kafka-adapter-on-gf-system.md) · [`gf-sales-events.md`](../events/gf-sales-events.md) §2bis.
>
> **13/14 trường payload Driver+ ánh xạ vào cột ĐÃ TỒN TẠI** của `booking` / `booking_details` (bảng mapping đầy đủ: `gf-sales-events.md` §3.8) — chỉ 2 cột dưới đây là mới.

### 2ter.1 `booking` — additive columns

| Column | Type | Nullable | Description | Cite |
|---|---|---|---|---|
| `cancel_source` | VARCHAR(20) | YES | Nguồn hủy — ghi cho **mọi** booking chuyển `CANCELLED`/`NO_SHOW`, **không phân biệt nguồn tạo** (phục vụ audit nội bộ). 3 giá trị: `DRIVERPLUS_USER` \| `GARAGE_INTERNAL` \| `NO_SHOW_AUTO`. NULL với booking chưa hủy | `BR-BOOK-023` · `FEAT-BOOK-DRIVERPLUS-OUTBOUND` AC-4 |
| `driverplus_service_type` | VARCHAR(50) | YES | "Loại dịch vụ" macro do Driver+ gửi — lưu **nguyên văn** 1 trong 3 giá trị `Car Spa` \| `Bảo dưỡng` \| `Sửa chữa`. **Độc lập hoàn toàn** với `service_type` (enum `ServiceType` nội bộ GMS) và danh mục `gf-erp-mdm` — KHÔNG map, KHÔNG liên kết. NULL với booking không phải nguồn Driver+ | `FEAT-BOOK-DRIVERPLUS-INBOUND` AC-2, **AC-3** · `FEAT-DP-034` §7 |

**Index bổ sung**:

| Index | Cột | Mục đích | Cite |
|---|---|---|---|
| `idx_booking_tenant_lead_source_status` | `(tenant_id, lead_source, status)` | Cover truy vấn vận hành "rà soát booking nguồn Driver+ theo trạng thái" (INBOUND AC-8 ghi ngoại lệ để đội vận hành rà soát) + gate phạm vi publish outbound theo nguồn (`gf-sales-events.md` §2bis.1 D11). Tenant-prefix bắt buộc | INBOUND AC-8 · OUTBOUND EC-2 |

> **Nợ kỹ thuật đã biết (không phát sinh từ W07)**: các index hiện hữu `idx_booking_status(status)`, `idx_booking_booked_at(booked_at)`, `idx_booking_lead_source(lead_source)`, `idx_booking_driver_id(driver_id)` **không** có prefix `(tenant_id, ...)` dù `booking` là bảng tenant-scoped. W07 **không** sửa (ngoài phạm vi, cần đo tác động query plan trên prod) nhưng index mới của W07 tuân thủ đầy đủ.

### 2ter.2 Migration

`gf-sales` dùng **hybrid**: bảng chính do Hibernate `ddl-auto=update` tạo/cập nhật; Flyway bổ sung table/function/index (xem §4). Áp đúng pattern đang chạy:

| Bước | Cơ chế | Nội dung |
|---|---|---|
| 1 | JPA entity (`ddl-auto=update`) | Thêm 2 field `cancelSource` + `driverPlusServiceType` vào `Booking` entity → Hibernate tự `ALTER TABLE ADD COLUMN` (nullable, an toàn với dữ liệu cũ) |
| 2 | Flyway `V1.0.{N+1}__add_booking_driverplus_columns_and_index.sql` | `CREATE INDEX IF NOT EXISTS idx_booking_tenant_lead_source_status ON booking (tenant_id, lead_source, status);` + `COMMENT ON COLUMN` cho 2 cột mới. Additive, **KHÔNG** rewrite migration cũ (Gotcha #9) |

**Backfill**: KHÔNG backfill `cancel_source` cho booking đã hủy trước W07 — để `NULL`. Lý do: (a) không suy được nguồn hủy lịch sử một cách tin cậy; (b) payload outbound chỉ áp cho booking hủy **sau** cutover; (c) `BR-BOOK-023` không yêu cầu hồi tố.

### 2ter.3 Enum bổ sung

| Enum/constant | Giá trị | Cite |
|---|---|---|
| `CancelSource` *(mới)* | `DRIVERPLUS_USER`, `GARAGE_INTERNAL`, `NO_SHOW_AUTO` | `BR-BOOK-023` (liệt kê đủ 3, verbatim) |
| `InboxEventType` | **KHÔNG đổi** — tái dùng `BOOKING_CREATED_FROM_DRIVER_PLUS`, `BOOKING_CANCELLED_BY_DRIVER` sẵn có | KG `gf-sales` |
| `BookingMessageStep` | `+ BOOKING.CANCEL.RESPONSE` (step outbound mới) | `gf-sales-events.md` §3.9bis |

> **`ServiceType` (enum nội bộ) KHÔNG đổi** — giá trị hiện tại không được document trong KG/data-model; W07 **không** đụng tới, và **không** map 3 giá trị macro Driver+ vào enum này (AC-3). DEV verify enum values từ source khi implement nếu cần render song song 2 loại "loại dịch vụ" trên cùng màn.
...
|---|---|---|
| 2026-08-11 | v1.8 | **Dọn dangling reference (per user sonhoang, `/warm-up gf-sales` GAP-W07-GSL-04)**: xóa cụm "Flag vào Open Questions" (§2ter.1, nợ tenant-prefix index) và "xem Open Questions" (§2ter.3, `ServiceType` enum note) — file này không có section "Open Questions" nào, 2 tham chiếu là dangling pointer cosmetic. Nội dung câu văn giữ nguyên, tự đủ nghĩa không cần section riêng. Không đổi nghiệp vụ/schema. |
| 2026-08-05 | v1.7 | **W07 Driver+ integration rewrite (DESIGN)** — thêm §2ter: 2 cột additive trên `booking` (`cancel_source` VARCHAR(20) per `BR-BOOK-023` áp cho **mọi** booking hủy không phân biệt nguồn; `driverplus_service_type` VARCHAR(50) lưu **nguyên văn** 3 giá trị macro D+, **KHÔNG** map vào enum `ServiceType` nội bộ per INBOUND AC-3 + `FEAT-DP-034` §7) + 1 index mới `idx_booking_tenant_lead_source_status (tenant_id, lead_source, status)` (tenant-prefix). §2ter.2 migration hybrid (JPA `ddl-auto=update` cho cột + Flyway `V1.0.{N+1}` cho index, additive — Gotcha #9) + quyết định **KHÔNG backfill** `cancel_source` lịch sử. §2ter.3 enum `CancelSource` mới (3 giá trị verbatim) + `BookingMessageStep` +`BOOKING.CANCEL.RESPONSE`; ghi rõ `InboxEventType` **không đổi** (tái dùng 2 giá trị sẵn có). Ghi nhận **nợ kỹ thuật** index `booking` hiện hữu thiếu tenant-prefix (không phát sinh từ W07, không sửa trong wave này). Frontmatter `depends_on` +ADR-029. **KHÔNG đụng**: §1 ERD, §2 toàn bộ entity baseline (13/14 trường payload D+ dùng cột đã có), §2bis Insurance, §3, §4, §5. v1.6 → v1.7. |
