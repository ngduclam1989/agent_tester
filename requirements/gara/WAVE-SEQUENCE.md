...
scope: "ep-insurance-settlement + ep-inventory-v2"
last_reviewed: "2026-07-28"  # v22 W06 version cite sync — gf-inventory-api.md v71→v72 (NF-02 fixed, drift-check re-verify 2026-07-28 verdict CONSISTENT). KHÔNG đụng nội dung khác §Wave 6. # v21 W06 full rebuild sync (/gen-wave-plan cascade PKG-W06 v2→v3) — §Wave 6 Boundaries row add `gf-accounting` (PRC master NEW boundary) + narrow `garage-mobile` note "chỉ FEAT-STK-LIST-V2"; Entry/Exit Criteria rewrite reflect ADR-027 v5 + ADR-028 v4 ACCEPTED (engine BQGQ + async HTTP 202 + Temporal `PRC_TASK_QUEUE` embed gf-accounting, KHÔNG gf-inventory-worker); P2.4 Cross-boundary contract row cite ADR-027/028 + INTEG-EXT-gf-accounting-gf-inventory; Review parallel row add gf-accounting; P2.5 đóng 2 row NEED CONFIRMATION cũ ("ADR engine BQGQ" + "PRC sync/async") → ✅ CLOSED với quyết định ratified. KHÔNG đụng PART I Insurance + W01-W05 sections + §1..§P2.3 flow + Change Log historical entries. # v20 W05 §Boundaries scope sync (CR-20260720-01 SELF_APPROVED MINOR) — line 543 add `gf-purchase` + `gf-hrms` boundary chính + upgrade `gf-sales` từ "**đọc** gf-sales (đối soát SO)" read-only footnote → boundary chính. Root cause: §Wave 5 row stuck at v11 pivot baseline chưa cascade khi PKG-W05 v11 (2026-07-16) promote gf-sales/gf-purchase từ read-only → producer per ADR-026 v2 (producer FF-gate + V2 topic route) + thêm gf-hrms per ADR-025 v1 (staff lookup-by-iam). Hệ quả pre-fix: `infra/wave_scope.py` regex parse §Boundaries row → 5 boundary → `/infra-up 05` thiếu `gf-hrms` + `gf-purchase` (user ninhnguyen identified 2026-07-20 Q&A). Sync với `Execution/work-packages/PKG-W05-inventory-receipt-delivery.md v13` §1 Overview line 25 canonical. `gf-accounting` (lock-check ADR-021) KHÔNG add — runtime pre-existing W04 dep, user ninhnguyen decision 2026-07-20 out-of-scope W05 infra derivation. **KHÔNG đụng** §Wave 5 Features/Entry Criteria/Exit/Demo + §P2.5 + PART I Insurance + W03/W04/W06 sections + Change Log historical entries. # v19 W05 Gap #4 extend cascade — mobile List có Post/Unpost inline row-action mobile-only (user bachho confirm 2026-07-15). Extend v18 wording (chỉ Detail có Post/Unpost) sang cả List. §Wave 5 Entry Criteria + §P2.5 row Figma mobile W05 subblock update: "2 LIST read-only + 2 DETAIL có Post/Unpost" → "2 LIST có Post/Unpost inline row-action (mobile-only, khác web) + 2 DETAIL có Post/Unpost per AC-5/6". Web List canonical UNCHANGED (Sửa/Xóa/In row-action per FEAT LIST-V2 AC-7). Mirror PKG-W05 v7 + LAUNCH-CHECKLIST v12. **KHÔNG đụng** scope + Features + Boundaries + Exit + Demo + Change Log historical entries. # v18 W05 Gap #4 cascade (user bachho triggered 2026-07-15 sau khi discover PKG-W05 v4 "mobile 4 màn read-only" contradict FEAT-*-DETAIL-V2 AC-5/6 canonical có Post/Unpost). Sửa 3 nơi wording (không đụng scope/Feature/Entry structure): (a) §Wave 5 Entry Criteria bullet Figma — "4 FEAT LIST + DETAIL read-only" → "4 màn: 2 LIST có Post/Unpost inline row-action (mobile-only, khác web) + 2 DETAIL có Post/Unpost per AC-5/6"; (b) §P2.5 row Figma mobile — same wording update W05 subblock; (c) §W06 Entry Criteria bullet Figma — mirror wording nếu cùng phrasing. Root cause + source-of-truth: FEAT-IR-DETAIL-V2 + FEAT-ID-DETAIL-V2 AC-4/5/6 (Business Authority owned) — nút "Ghi sổ kho" / "Bỏ ghi sổ kho" trên màn Detail có Figma mobile node đã gắn (`21629:24082` / `21629:28663`), KHÔNG có platform gate. Danh sách web-only ops mở rộng thêm PRINT/EXPORT (5 op web-only tổng: CREATE/EDIT/DELETE/PRINT/EXPORT × 2 = 10 FEAT). **KHÔNG đụng** PART I Insurance + W03/W04/W06 sections + §1..§P2.4 flow + §Wave 5 Features/Boundaries/Exit/Demo. Cascade PKG-W05 v5→v6 + LAUNCH-CHECKLIST v10→v11. v5 mobile detail work bổ sung +4h effort (17h → 21h) ghi trong PKG-W05 §4.1. # v17 W05 Gap #3 resolve cascade — §P2.5 close 1 row: "Auto-create phiếu từ PO/SO scope W05 hay defer" OPEN → ✅ CLOSED (Gap #3 batch full include W05 per user bachho Q1=B decision 2026-07-15, không CR raise). Spec: `gf-inventory-api.md v57 §3e` (W05-AR1 `POST /protected/receipts-v2/from-po` + W05-AD1 `POST /protected/deliveries-v2/from-so` protected endpoints worker-facing, x-api-key auth, bypass staff-eligibility + lock-check, Idempotency-Key deterministic + inbox `processed_events` ADR-004 dedup, 4-nhóm state input per BR-IRV2-033 v35 + BR-IDV2-032 v32) + `INTEG-EXT-gf-inventory.md v16 §13d` (worker-consumer contract: consumer table 3 rows V2 Receipt/V2 Delivery/V1 fallback + auth + route pseudocode flag-aware per-tenant via TenantSubscriptionCacheService + failure handling 5xx/409/422/400 + V1→V2 transition + 5 forbidden rules + 2 soft questions) + `gf-inventory-HLD.md v25 §1` (subsystem callout sub-bullet mới sau Feature flag). Worker `gf-inventory-worker` route logic per-tenant flag check + 2 new client methods `createReceiptV2FromPO()` + `createDeliveryV2FromSO()` extend existing `InventoryClient` (`services/gf-inventory-worker/src/main/java/com/actechx/gf/worker/adapter/client/InventoryClient.java`). V1 Protected controllers exempt `@FeatureOff` per Q2=A (giữ callable cho legacy tenants + fallback). BQGQ W06 chỉ đọc V2 tables per Q3=A. **Cascade Product follow-up needed** (EP-INVENTORY-RECEIPT-V2 §5.2 + EP-INVENTORY-DELIVERY-V2 §5.2 wording clarify V1 Protected exempt scope) — BA authorize required, không thuộc Architecture edit scope trong batch này per user preference memory `scope-gap-resolve` 2026-07-15. 1 row còn OPEN: engine BQGQ W06 + PRC sync/async W06. Cascade PKG-W05 v3→v4 + LAUNCH-CHECKLIST v9→v10. # v16 W06 cross-wave state-matrix future-proof — §Wave 6 Features row expand thêm ghi chú `FEAT-INV-MOBILE-MENU` (cross-wave state-matrix update) song song 8 FEAT PRC+STK, giữ nguyên count. Cascade PKG-W06 v1→v2 §2.2 Technical Scope + figma-links.yaml add block `waves["06"]` (same node 21729:24201 pattern W03/W04/W05). Mục đích: `/gen-wave-plan W06` cycle sau đọc §Wave 6 sẽ tự pick up FEAT-INV-MOBILE-MENU (không bị bỏ sót như W04 v6 retrofit 2026-07-02). # v15 W05 Gap #2 resolve cascade — §P2.5 close 1 row: "so-summary verb lệch" OPEN → ✅ CLOSED (bundle: ADR-024 v5 + INTEG-EXT-gf-inventory v15 §13c rewrite 8 nơi + gf-inventory-HLD v24 §1/§5/§6b.7 update 8 nơi; verify runtime code services/gf-sales + services/gf-inventory phát hiện D2 endpoint cite sai DIRECTION, không chỉ verb; endpoint đúng = GET /protected/v1/service-orders/{tenantId}/detail/{code} existing tại gf-sales, gf-inventory GfSalesClient đã có sẵn từ trước W05). KHÔNG đụng 2 row còn OPEN (auto-create phiếu PO/SO Gap #3 · engine BQGQ W06 · PRC sync/async W06). Cascade PKG-W05 v3 unchanged (§3 Entry Criteria đã note ADR-024 v4 ACCEPTED — nay bump v5 semantic tương thích). Cascade LAUNCH-CHECKLIST v9. # v14 W05 Gap #1 ratify batch cascade — §P2.5 close 2 row: (a) "ADR — lock kỳ" PARTIAL → CLOSED (ADR-023 v3 ACCEPTED §D5 cover slip lock-check); (b) "ADR-023/024/025 ratify" OPEN → CLOSED (bundle với INTEG-BFF-gf-hrms v2 + ERROR-CODE-REGISTRY v24 §4a add ERR-HRMS-STAFF-NOT-ELIGIBLE). 3 row còn OPEN (so-summary verb Gap #2 · auto-create phiếu PO/SO Gap #3 · engine BQGQ W06) không đụng. KHÔNG đụng PART I Insurance + W03/W04/W06 sections + §1..§P2.4 flow. Cascade PKG-W05 v3 + LAUNCH-CHECKLIST v8.
---
...
>
> Phương pháp planning: **M01 Vertical-Slice Wave Planning** (governing cố định — `Plan/PLANNING-PLAYBOOK.md`). Timebox cố định **5 ngày làm việc/wave**. Cadence epic-driven; wave đánh số liên tục (Insurance W01–W02 → Inventory V2 W03–W06).
