...
- [ ] **AC-1b**: Mở form từ phiếu nhập/xuất kho (kế thừa SKU, mode **modal inline**)
  ...
  - **Container**: mode **modal inline** (không rời form phiếu) — reuse **cùng component form** với AC-1 (full 4 tab, cùng validation/logic); khác container: modal overlay thay vì màn full-page. Form phiếu giữ nguyên phía sau modal.
  - Khi: dòng có **SKU chưa mapping mã nội bộ nào**.
  - Thì: hệ thống mở modal và **gắn sẵn SKU đó vào tab "Mã SKU"** (mã SKU + tên SKU). **"Mã sản phẩm nội bộ"** và **"Tên sản phẩm"** đều **để trống — nhập tay** (không seed từ tên SKU).
  - Khi: dòng có **SKU đã mapping mã nội bộ khác** (1 SKU chỉ thuộc 1 mã — BR-CAT-PROD-013).
  - Thì: **KHÔNG** gắn sẵn SKU vào tab "Mã SKU"; modal mở **trống** (coi như người dùng tạo mã nội bộ mới để gắn với **SKU khác**).
  - Khi: dòng **chưa chọn SKU**, hoặc mở từ danh mục mã sản phẩm.
  - Thì: form mở **trống** như bình thường (AC-1 mode full-page danh mục; hoặc AC-1b mode modal khi mở từ phiếu — cùng behavior form trống).
  - **Sau khi Lưu thành công trong modal**: mã mới ("Đang hoạt động") được trả về form phiếu và **tự chọn vào dòng** trigger việc mở modal; đóng modal, quay lại form phiếu giữ nguyên dữ liệu đã nhập. **Hủy modal**: không tạo mã, đóng modal, quay lại form phiếu giữ nguyên.
...
| 2026-07-13 | 14 | Business Authority (BA in-session review W05 chuẩn bị) | **AC-1b thêm container mode "modal inline"** (cascade BR-IRV2-027 v32 + BR-IDV2-027 v28). Khi mở form từ dropdown "+ Tạo mới mã nội bộ" ở phiếu nhập/xuất kho, hệ thống render form **trong modal inline** (không rời form phiếu), **reuse cùng component** với AC-1 (full 4 tab, cùng validation/logic) — khác container: modal overlay thay vì màn full-page. Sau lưu thành công: mã mới trả về form phiếu + tự chọn vào dòng trigger + đóng modal. Hủy modal: không tạo, giữ nguyên form phiếu. Pre-fill SKU (SKU chưa mapping) vẫn áp trong mode modal. Lý do: UX tốt hơn (không mất dữ liệu form phiếu), consistent với pattern "+ Thêm ĐVT quy đổi" đã là modal inline (BR-IRV2-029 / BR-IDV2-029). |
