...
**Mục đích:** Sinh test cases chi tiết theo chiến lược Risk-Based Testing (RBT).

**Agent phải:**
1. Đánh giá Risk Level cho mỗi Module/Chức năng:
   - **High Risk:**
     - **Function:** Test kỹ, nhiều cases bao gồm cả Happy Path và Unhappy Path của module đó.
     - **Phân quyền:** Kiểm tra phân quyền của các user khác nhau có quyền và không có quyền sử dụng chức năng (ví dụ: Admin, User được cấp quyền, User không được cấp quyền).
     - **Ảnh hưởng chức năng liên quan:** Các nghiệp vụ liên quan mật thiết đến chức năng chính được yêu cầu (ví dụ: các liên hệ CRUD liên quan tới bản ghi của function đó).
   - **Medium Risk:**
     - **Validate:** Kiểm tra validate của từng trường một (required, min/max, kiểm tra chéo dữ liệu khi nhập các loại dữ liệu khác như chữ, số, ký tự đặc biệt, ký tự có dấu, khoảng trắng, khoảng trắng ở đầu và cuối).
     - **UI:** Kiểm tra giao diện cho từng element UI một, thông báo rõ từng vị trí của element, nội dung, font chữ, màu sắc; các phần khác tính là behavior (ấn tab, shift tab, hover, focus, khi load web, khi resize).
...
2. Sinh test case với đầy đủ fields:
   ...
   - Expected Results (đánh số cụ thể 1, 2, 3... tương ứng 1-1 với từng Test Step)
   - Test Data (**phải cụ thể**, không dùng placeholder chung chung, ví dụ: "Nhập email: test_user_01@domain.com", không dùng "Nhập email hợp lệ")
   - Priority
3. Bao phủ đa dạng:
   - Happy Path
...
4. **Validation chuyên biệt từng trường (Field-Level Validation):**
   ...
   - Tham chiếu **Bảng Field-Level Validation** ở phần Mode QUICK để chọn validation phù hợp (với từng loại data riêng biệt đều phải có validate, không được gộp)
...
**Agent phải thực hiện chuẩn xác 2 bước sau:**

#### BƯỚC 1: TẠO FILE MARKDOWN TỔNG HỢP (Artifact)
Tạo 1 file Markdown duy nhất (ví dụ: `TC_[MODULE].md` hoặc `test_cases_[MODULE].md`) chứa TOÀN BỘ các thông tin sau:
...
Ngay sau khi sinh và lưu xong file Markdown ở Bước 1, bạn (AI Agent) **BẮT BUỘC** phải tự động chạy lệnh Terminal trong Workspace để convert file Markdown đó sang file Excel (.xlsx).
...
**Output:** 
- Bảng Test Cases Markdown hoàn chỉnh dạng Artifact.
- File Excel (.xlsx) đã được tự động convert thành công.
