...
3. **Đọc hiểu sâu tài liệu yêu cầu (sử dụng file `.md` đã được chuyển đổi nếu áp dụng):**
...
4. **Vẽ nháp Diagram (nếu thiếu các sơ đồ nghiệp vụ cần thiết):**
...
5. Tóm tắt scope kiểm thử, bối cảnh nghiệp vụ, các thông tin trường dữ liệu đã trích xuất, và danh sách các diagram đã vẽ nháp (nếu có).
6. **Chờ user xác nhận** trước khi sang Bước 2.

**Output:** Xác nhận hiểu bối cảnh + tóm tắt scope kiểm thử + các diagram đã vẽ nháp (nếu có).

---

### Bước 2: Analysis & QnA (Phân tích yêu cầu)

**Mục đích:** Phân tích tài liệu và các diagram để phát hiện điểm mờ, thiếu sót, mâu thuẫn, đồng thời bóc tách toàn bộ kịch bản nghiệp vụ rẽ nhánh.

**Agent phải:**

1. **Đọc và phân tích các Diagram:**
...
4. **Đặt câu hỏi Q&A có cấu trúc rõ ràng:**
...
5. **DỪNG LẠI — Chờ user trả lời** các câu hỏi Q&A trước khi chuyển sang Bước 3.

**Output:** Danh sách luồng + Các kịch bản rẽ nhánh theo diagram + Ambiguities + Câu hỏi Q&A.
...
2. **Sinh test case với đầy đủ các trường (fields):**
   ...
   - Priority (Critical / High / Medium / Low)
3. **Bao phủ đa dạng:** Happy Path (gồm case nhập full thông tin và case chỉ nhập thông tin bắt buộc tối thiểu), Negative Path (giá trị biên, vượt ký tự, đặc biệt là kịch bản validate thiếu dần từ ít đến nhiều các thông tin bắt buộc), Edge Cases.
4. **Kỹ thuật thiết kế test case:** Equivalence Partitioning (EP), Boundary Value Analysis (BVA), Decision Table (Tổ hợp điều kiện), State Transition (Chuyển đổi trạng thái).
