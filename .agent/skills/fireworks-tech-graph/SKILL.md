...
Bốn tập lệnh hỗ trợ trong thư mục `.agent/skills/fireworks-tech-graph/scripts/` cung cấp khả năng sinh và xác thực SVG ổn định:

### 1. `generate-diagram.sh` - Xác thực SVG + xuất PNG
```bash
./.agent/skills/fireworks-tech-graph/scripts/generate-diagram.sh -t architecture -s 1 -o ./output/arch.svg
```
- Xác thực tệp SVG hiện có
- Xuất tệp PNG sau khi xác thực
- Ví dụ: `./.agent/skills/fireworks-tech-graph/scripts/generate-diagram.sh -t architecture -s 1 -o ./output/arch.svg`

### 2. `generate-from-template.py` - Tạo sơ đồ SVG ban đầu từ bản mẫu (template)
```bash
python3 ./.agent/skills/fireworks-tech-graph/scripts/generate-from-template.py architecture ./output/arch.svg '{"title":"My Diagram","nodes":[],"arrows":[]}'
```
...
```bash
./.agent/skills/fireworks-tech-graph/scripts/validate-svg.sh <svg-file>
```
...
```bash
./.agent/skills/fireworks-tech-graph/scripts/test-all-styles.sh
```
...
4. **Tải tham chiếu style** — luôn tải `.agent/skills/fireworks-tech-graph/references/style-1-flat-icon.md` trừ khi người dùng chỉ định loại khác; tải tệp `.agent/skills/fireworks-tech-graph/references/style-N.md` tương ứng để lấy các token màu sắc và mẫu SVG chính xác
5. **Ánh xạ các node với hình dạng** — sử dụng Từ vựng hình dạng bên dưới
6. **Kiểm tra nhu cầu biểu tượng** — tải `.agent/skills/fireworks-tech-graph/references/icons.md` cho các sản phẩm đã biết
...
**Nếu sử dụng `.agent/skills/fireworks-tech-graph/scripts/generate-from-template.py`**:
...
Tải `.agent/skills/fireworks-tech-graph/references/style-N.md` để lấy chính xác các token màu sắc và mẫu SVG.

## Lựa chọn style

**Mặc định**: Style 1 (Flat Icon) cho hầu hết các sơ đồ. Tải `.agent/skills/fireworks-tech-graph/references/style-diagram-matrix.md` để biết các khuyến nghị cụ thể từ style sang loại sơ đồ.
