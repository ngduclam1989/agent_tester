"""Thêm bình luận (comment) vào tài liệu DOCX.

Cách dùng:
    python comment.py unpacked/ 0 "Nội dung bình luận"
    python comment.py unpacked/ 1 "Nội dung trả lời" --parent 0

Văn bản phải là XML được escape trước (ví dụ: &amp; cho &, &#x2019; cho dấu nháy thông minh).

Sau khi chạy, hãy thêm các điểm đánh dấu vào document.xml:
  <w:commentRangeStart w:id="0"/>
  ... nội dung được bình luận ...
...
Thêm vào document.xml (các điểm đánh dấu phải là con trực tiếp của w:p, không bao giờ nằm trong w:r):
...
Lồng các điểm đánh dấu bên trong điểm đánh dấu của cha {pid} (các điểm đánh dấu phải là con trực tiếp của w:p, không bao giờ nằm trong w:r):
...
# Chuyển các dấu ngoặc kép thông minh thành thực thể XML
...
def add_comment(
    ...
    if not word.exists():
        return "", f"Lỗi: Không tìm thấy {word}"
    ...
    if parent_id is not None:
        ...
        if not parent_para:
            return "", f"Lỗi: Không tìm thấy bình luận cha {parent_id}"
        _append_xml(
            ext,
            "w15:commentsEx",
    ...
    action = "trả lời" if parent_id is not None else "bình luận"
    return para_id, f"Đã thêm {action} {comment_id} (para_id={para_id})"


if __name__ == "__main__":
    p = argparse.ArgumentParser(description="Thêm bình luận vào tài liệu DOCX")
    p.add_argument("unpacked_dir", help="Thư mục DOCX đã giải nén")
    p.add_argument("comment_id", type=int, help="ID bình luận (phải là duy nhất)")
    p.add_argument("text", help="Nội dung bình luận")
    p.add_argument("--author", default="Claude", help="Tên tác giả")
    p.add_argument("--initials", default="C", help="Chữ viết tắt tên tác giả")
    p.add_argument("--parent", type=int, help="ID bình luận cha (để trả lời)")
    args = p.parse_args()
