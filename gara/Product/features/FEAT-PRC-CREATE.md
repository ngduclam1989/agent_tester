...
**As** chủ garage / kế toán, **I want** thực hiện tính giá vốn xuất kho theo kỳ + kho + mã chọn (phương pháp BQGQ cuối kỳ), **so that** giá vốn các phiếu xuất trong kỳ được điền và giá trị tồn được chốt chính xác.
...
- [ ] **AC-1**: Mở form
  ...
  - Thì: hệ thống mở màn **"Thực hiện tính giá xuất kho"** với mô tả **"Form tính lại giá vốn cho các mặt hàng xuất kho trong kỳ, theo Kho và phương pháp bình quân cuối kỳ."**, mục **"Thông tin kỳ tính giá"** + bảng **"Vật tư hàng hóa cần tính giá"**, nút **"Đóng"** / **"Thực hiện"**.
...
- [ ] **AC-4**: Chọn vật tư hàng hóa
  ...
  - Khi: hệ thống lấy danh sách mã (cả 2 cách).
  - Thì: nguồn mã = **danh mục vật tư hàng hóa (Mã sản phẩm nội bộ)** đã lọc **"Phương pháp tính giá" = "Bình quân cuối kỳ"** (BR-PRC-012; thiết lập tại `FEAT-CAT-PROD-CREATE/EDIT`). Mã phương pháp khác không xuất hiện.
  - Khi: chọn **"Tất cả mã"**.
  - Thì: hệ thống **tự lấy mọi mã** (thoả phương pháp BQGQ) thuộc kho đã chọn vào phạm vi tính — không cần thêm thủ công (bảng "Vật tư hàng hóa cần tính giá" hiển thị toàn bộ mã được lấy, có thể loại bớt từng dòng).
  - Khi: chọn **"Chọn mã cụ thể"**.
  - Thì: hệ thống cho thêm mã qua nút **"Thêm phụ tùng"** vào bảng "Vật tư hàng hóa cần tính giá" (dropdown chỉ liệt kê mã BQGQ từ danh mục — AC-6b).

### Nhóm B — Bảng vật tư cần tính

- [ ] **AC-5**: Cột bảng vật tư
  - Tại: bảng **"Vật tư hàng hóa cần tính giá"**.
  - Thì: hệ thống hiển thị: **"STT"**, **"Mã nội bộ"**, **"Tên hàng hóa"**, **"ĐVT chính"**, **"Có phát sinh xuất"** (số phiếu xuất của mã trong kỳ/kho — thông tin), **"Lần tính gần nhất"** (ngày tính gần nhất hoặc "Chưa tính trong kỳ" — thông tin), **"Thao tác"** (xóa dòng).

- [ ] **AC-6**: Thêm / xóa mã
  - Tại: nút **"Thêm phụ tùng"** / icon xóa trên dòng.
  - Thì: thêm mã vào bảng / loại mã khỏi bảng tính giá.

- [ ] **AC-6b**: Chỉ mã có phương pháp tính giá = BQGQ
  - Tại: khi chọn/thêm mã vào bảng tính giá.
  - Khi: chủ garage thêm mã.
  - Thì: hệ thống **chỉ cho chọn mã có "Phương pháp tính giá" = "Bình quân cuối kỳ"** (thiết lập ở danh mục Mã sản phẩm nội bộ — `FEAT-CAT-PROD-CREATE/EDIT`). Mã đặt phương pháp khác (Đích danh / Nhập trước xuất trước — mở rộng tương lai) **không thuộc** lần tính BQGQ này.
...
- [ ] **AC-8**: Cập nhật kết quả
  - Tại: sau khi tính.
  - Thì: hệ thống **(1) Cập nhật phiếu xuất** — điền **giá vốn = làm tròn(Đơn giá BQ × SL) về đồng** vào **toàn bộ phiếu xuất trong kỳ** (đang 0); **(2) Cập nhật sổ tồn** — tính lại giá trị tồn của (mã + kho) **từ kỳ tính trở đi** → báo cáo tồn / NXT tự đúng (BR-PRC-005); **(3)** ghi **log** lần chạy (tài khoản + ngày giờ + số mã). **Tồn cuối kỳ (SL, GT)** làm điểm đầu cho lần tính sau.

- [ ] **AC-8b**: Lưu phiếu trước, chạy giá nền (cập nhật dần)
  - Tại: nút **"Thực hiện"**.
  - Khi: chủ garage bấm thực hiện.
  - Thì: hệ thống **lưu ngay bản ghi lần tính** (kỳ / kho / khoảng ngày / phạm vi mã / người thực hiện / thời điểm) với **trạng thái "Đang tính"** (phiếu xuất hiện ngay, không chờ tính xong) → chạy tính BQGQ **từng mã ở nền** (có thể lâu); tính xong mã nào → **cập nhật trạng thái + kết quả mã đó** (giá vốn phiếu xuất + giá trị sổ tồn theo AC-8); xong toàn bộ → **chốt trạng thái lần tính** = "Thành công" / "Hoàn thành có lỗi" (BR-PRC-016, BR-PRC-014).
...
- [ ] **AC-9b**: Cảnh báo kỳ sau cần tính lại
  - Tại: nút **"Thực hiện"** / sau khi tính.
...
- [ ] **AC-10**: Mã lỗi (chạy giá lỗi)
  - Tại: sau khi tính.
  - Khi: trong kỳ **SL tồn của mã bị âm** (xuất vượt tồn, tại thời điểm bất kỳ / cuối kỳ) → lý do lỗi **"Do tồn âm"** (`ERR-INV-030`).
  - Thì: hệ thống đánh dấu mã **"Lỗi"** — **không** cập nhật giá vốn phiếu + **không** cập nhật giá trị tồn cho mã đó; các mã khác vẫn hoàn tất; đưa mã vào bảng "Sản phẩm chạy giá lỗi" với lý do **"Do tồn âm"** + hướng xử lý (Xem `FEAT-PRC-DETAIL`).
  - Ghi chú: tồn âm **đã bị chặn point-in-time ở mọi thao tác chạm tồn** (tạo/sửa/lùi ngày/xóa phiếu, import/xóa OB, bỏ ghi sổ) → "Do tồn âm" ở đây là **kiểm tra phòng vệ (invariant)**, về nguyên tắc không xảy ra (BR-PRC-007). Lý do **"Lệch hạch toán"** (`ERR-INV-031`) thuộc **[MỞ RỘNG TƯƠNG LAI]** — hạch toán chưa triển khai nên **hiện chưa bắt**. Mẫu số = 0 → đơn giá BQ = 0, **không** là lỗi (BR-PRC-001/007).

### Nhóm D — Đóng & phân quyền

- [ ] **AC-11**: Đóng — nút **"Đóng"** đóng form, không tính.
...
- Lấy mã cần tính (có phát sinh xuất + lần tính gần nhất): Query `[PROPOSED] GetItemsForCogs`.

## 5. Business Rules

- **BR-PRC-001 .. 017** (xem `BR-GF-INVENTORY-ACCOUNTING-PERIOD` §2.2): công thức BQGQ (mẫu số=0→0), **tồn đầu = tồn kho đến "Từ ngày" − 1 (SL + GT)**, cập nhật phiếu xuất + **cập nhật sổ tồn**, mã lỗi, chọn kỳ khóa ngày, theo mã+kho+gara, **tính lặp khi có phiếu trả tự tham chiếu (BR-PRC-017)**.
...
- **BR-PRC-012**: chỉ tính mã có phương pháp = Bình quân cuối kỳ.
...
- **EC-4**: Mẫu số (SL tồn đầu + SL nhập) = 0 → đơn giá BQ = 0 (không phải lỗi). **SL tồn của mã trong kỳ bị âm** (xuất vượt tồn) → mã vào bảng "Sản phẩm chạy giá lỗi" với lý do **"Do tồn âm"** (`ERR-INV-030` — AC-10).
- **EC-5**: Tính lại một kỳ đã có kỳ sau được tính → kỳ sau thành cũ; hệ thống cảnh báo, người dùng tự tính lại kỳ sau (BR-PRC-015).
- **EC-6**: Mã có dòng phiếu **"Nhập hàng bán bị trả lại" "Tự nhập giá" KHÔNG tích** (đơn giá để hệ thống cập nhật) tham chiếu phiếu **Xuất bán cùng kỳ chưa tính** → GT nhập phụ thuộc giá vốn xuất (output BQGQ) → hệ thống **tính lặp đến khi đơn giá BQ hội tụ** (BR-PRC-017): mỗi vòng tính giá vốn xuất bán **trước** rồi cập nhật GT phiếu trả → tính lại. Dòng **"Tự nhập giá" tích** (nhập đơn giá tay) → không lặp.

## 7. Out of Scope

- Xem kết quả / bảng lỗi → `FEAT-PRC-DETAIL`. Tính lại → `FEAT-PRC-RECALC`. Xóa log → `FEAT-PRC-DELETE`.
