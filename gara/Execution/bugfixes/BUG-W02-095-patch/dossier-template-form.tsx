...
type AcceptanceVariantProps = {
  variant: "acceptanceRecord";
  prefill: DossierTemplatePrefill;
  onPrint?: () => void;
  printLabelOverride?: string;
  printDisabled?: boolean;
  onValuesChange?: (values: AcceptanceFormInput) => void;
};

type AuthorizationVariantProps = {
  variant: "paymentAuthorization";
  prefill: DossierTemplatePrefill;
  onPrint?: () => void;
  printLabelOverride?: string;
  printDisabled?: boolean;
  onValuesChange?: (values: AuthorizationFormInput) => void;
};

type DossierTemplateFormProps =
  | AcceptanceVariantProps
  | AuthorizationVariantProps;
...
export const buildAcceptanceFromPrefill = (
  prefill: DossierTemplatePrefill,
): AcceptanceFormInput => {
...
export const buildAuthorizationFromPrefill = (
  prefill: DossierTemplatePrefill,
): AuthorizationFormInput => {
  const context = buildPrefillContext(prefill);
  const defaults: Record<string, unknown> = {
...
interface AcceptanceValuesWatcherProps {
  control: UseFormReturn<AcceptanceFormInput>["control"];
  onValuesChange?: (values: AcceptanceFormInput) => void;
}

const AcceptanceValuesWatcher = ({
  control,
  onValuesChange,
}: AcceptanceValuesWatcherProps) => {
  const watched = useWatch({ control });

  useEffect(() => {
    if (!onValuesChange) return;
    onValuesChange(watched as unknown as AcceptanceFormInput);
  }, [watched, onValuesChange]);

  return null;
};

interface AuthorizationValuesWatcherProps {
  control: UseFormReturn<AuthorizationFormInput>["control"];
  onValuesChange?: (values: AuthorizationFormInput) => void;
}

const AuthorizationValuesWatcher = ({
  control,
  onValuesChange,
}: AuthorizationValuesWatcherProps) => {
  const watched = useWatch({ control });

  useEffect(() => {
    if (!onValuesChange) return;
    const raw = watched as unknown as AuthorizationFormInput;
    onValuesChange(normalizeAuthorizationGetValues(raw));
  }, [watched, onValuesChange]);
...
>((props, ref) => {
  const { variant, prefill, onPrint, printLabelOverride, printDisabled } =
    props;

  const acceptanceDefaults = useMemo(
    () => buildAcceptanceFromPrefill(prefill),
    [prefill],
  );
  const authorizationDefaults = useMemo(
    () => buildAuthorizationFromPrefill(prefill),
    [prefill],
  );
  ...
  if (variant === "acceptanceRecord") {
    const acceptanceOnValuesChange = (props as AcceptanceVariantProps)
      .onValuesChange;
    return (
      <TemplateBody<AcceptanceFormInput>
        ...
        watcher={
          <AcceptanceValuesWatcher
            control={acceptanceForm.control}
            onValuesChange={acceptanceOnValuesChange}
          />
        }
      />
    );
  ...
  const authorizationOnValuesChange = (props as AuthorizationVariantProps)
    .onValuesChange;
  return (
    <TemplateBody<AuthorizationFormInput>
      ...
      printDisabled={printDisabled}
      watcher={
        <AuthorizationValuesWatcher
          control={authorizationForm.control}
          onValuesChange={authorizationOnValuesChange}
        />
      }
    />
  );
