...
| Total events | **2 outbound PROPOSED** (`AccountingPeriodClosed`, `AccountingPeriodReopened` — KHÔNG publish trong batch; ACTIVE flip = future wave) |
| Reliability | Transactional outbox (`outbox_events`) + scheduled retry (poll 5s, batch 100, max 3 retry, Redis singleton lock) — ADR-004. Sẵn sàng nhưng chưa wire cho event publish nào (PROPOSED = contract only). |
...
| Partition key | `AccountingPeriod-{periodCode}` (per-aggregate per `_CONVENTIONS §4`; `periodCode` auto-derived deterministic — xem data-model §2ter.1) |
